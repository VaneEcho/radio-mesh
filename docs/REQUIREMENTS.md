...
| Tektronix RSA306B USB 频谱分析仪 | USB 3.0 | `tekrsa-api-wrap` + `libRSA_API.so` | 第二优先，占位待实现 |
...
**数据压缩（时间维度）：**
- 时间窗口：1 分钟
- 聚合方式：每个频率点（25 kHz bin）取窗口内最大电平值
...
帧率控制：Edge 侧节流，默认 5 fps，可配置，0 = 不节流。
...
**Cloud 侧：**
- [ ] 频谱统计帧接收 API（解码 + 写 TimescaleDB）
- [ ] 简单历史查询 API（按站点+时间段取某频率点的电平序列）
- [ ] 频段规则数据库 + 管理页面（band_rules 导入 + CRUD）
- [ ] 前端基础页面（站点状态 + 历史电平折线图）
